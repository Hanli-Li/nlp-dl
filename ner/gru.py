...
logger = logging.getLogger("rnn.sequence.behavior")
...
class SequencePredictor(Model):
    ...
    def add_loss_op(self, preds):
        ...
        Functions tf.reduce_mean and tf.l2_loss can be useful.
    ...
    def add_training_op(self, loss):
        ...
        grad_var = zip(*grad_var_pairs)

        grad_clipped = list(grad_var[0])
        if self.config.clip_gradients:
            grad_clipped, norm = tf.clip_by_global_norm(grad_clipped, self.config.max_grad_norm)
        global_norm = tf.global_norm(grad_clipped)
        self.grad_norm = global_norm
        train_op = optimizer.apply_gradients(zip(tuple(grad_clipped), grad_var[1]))
...
if __name__ == "__main__":
    ...
    command_parser.add_argument('-o', '--output-prefix', type=str, default="sequence", help="Length of sequences to generate")
