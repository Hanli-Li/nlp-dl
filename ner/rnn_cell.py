...
logger = logging.getLogger("ner.rnn.cell")
...
class RNNCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        The code below implements an RNN cell using @inputs
        (x_t above) and the state (h_{t-1} above).
            - Define W_x, W_h, b to be variables of the apporiate shape
              using the `tf.get_variable' functions. Use
        ...
        with tf.variable_scope(scope):
            ...
            W_h = tf.get_variable("W_h", shape=[self._state_size, self._state_size], \
                                    initializer=tf.contrib.layers.xavier_initializer())
            b = tf.get_variable("b", shape=[self._state_size], initializer=tf.contrib.layers.xavier_initializer())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Tests the RNN cell')
