...
logger = logging.getLogger("ner.rnn.gru.cell")
...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    """
    Wrapper around RNN cell implementation, which allows to play 
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        The function implements an GRU cell using @inputs
        ...
        Remember to initialize matrices using the xavier
        ...
        # Scope variables in functions lest they be defined elsewhere!
        with tf.variable_scope(scope):
            init = tf.contrib.layers.xavier_initializer()
            
            U_z = tf.get_variable("U_z", shape=[self.input_size, self._state_size], initializer=init)
            W_z = tf.get_variable("W_z", shape=[self._state_size, self._state_size], initializer=init)
            ...
            U_o = tf.get_variable("U_o", shape=[self.input_size, self._state_size], initializer=init)
            W_o = tf.get_variable("W_o", shape=[self._state_size, self._state_size], initializer=init)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Tests the GRU cell')
