...
logger = logging.getLogger("ner.window")
...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    The function constructs a window from each word in the
    ...
    for sentence, labels in data:
        ...
        for i in xrange(window_size, n + window_size):
            vec = [item for sublist in sentence[i - window_size : i + window_size + 1] for item in sublist]
            windowed_data.append((vec, labels[i - window_size]))
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        data during training. Note that when "None" is in a placeholder's shape, batch sizes 
        can be flexible
    ...
    def add_embedding(self):
        ...
        Can use tf.nn.embedding_lookup for look-up, and tf.reshape to concatenate the vectors. 
        See following link to understand what -1 in a shape means.
    ...
    def add_prediction_op(self):
        ...
        Softmax is not applied to pred here. The softmax will instead be done in
        the add_loss_op function, which improves efficiency with 
        tf.nn.softmax_cross_entropy_with_logits

        When creating a new variable, use the tf.get_variable function
        as it allows specifying an initializer.

        Use tf.contrib.layers.xavier_initializer to initialize matrices.
        This is TensorFlow's implementation of the Xavier initialization trick used here.
    ...
    def add_loss_op(self, pred):
        ...
        Can use tf.nn.sparse_softmax_cross_entropy_with_logits and tf.reduce_mean
        Args:
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            ...
            print("""Welcome! You can use this shell to explore the behavior of your model.
                Please enter sentences with spaces between tokens, 
                e.g., input> Germany 's representative to the European Union 's veterinary committee.""")
            while True:
                # Create simple REPL
                try:
