...
def pad_sequences(data, max_length):
    ...
    For every sentence, labels pair in @data,
    ...
    for sentence, labels in data:
        ...
        if n >= max_length:
            ...
            sent = sentence + [zero_vector] * (max_length - n)
            lab = labels + [zero_label] * (max_length - n)
...
class RNNModel(NERModel):
    ...
    def __init__(self, helper, config, pretrained_embeddings, report=None):
        ...
        Config.max_length = self.max_length  # Just in case of making a mistake.
    ...
    def add_placeholders(self):
        ...
        data during training.  Note that when "None" is in a placeholder's shape, 
        batch sizes can be flexible.
    ...
    def add_embedding(self):
        ...
        """
        vecs = tf.Variable(self.pretrained_embeddings)
        embeddings = tf.nn.embedding_lookup(vecs, self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        This function:
            ...
            - When unrolling the loop, from the second iteration
              onwards, MUST call tf.get_variable_scope().reuse_variables() so as NOT to
              create new variables in the RNN cell.
            ...
            - Concatenate and reshape the predictions into a predictions
              tensor.
        Note: The function tf.pack (similar to np.asarray) is
        ...
        Note: The function tf.transpose and the perms
        ...
        b = tf.get_variable("b", shape=[self.config.n_classes], initializer=tf.contrib.layers.xavier_initializer())
        ...
        with tf.variable_scope("RNN"):
            for t in range(self.max_length):
                ...
                o_t, h_t = cell(x[:, t, :], h_t)
                o_t_drop = tf.nn.dropout(o_t, self.dropout_placeholder)
                y_t = tf.matmul(o_t_drop, U) + b
                preds.append(y_t)
        ...
        assert preds.get_shape().as_list() == [None, self.max_length, self.config.n_classes], \
            "predictions are not of the right shape. Expected {}, got {}".format([None, self.max_length, self.config.n_classes], preds.get_shape().as_list())
        return preds
    ...
    def add_loss_op(self, preds):
        ...
        This function computes averaged cross entropy loss for the predictions.
        Importantly, must ignore the loss for any masked tokens.

        Note: tf.boolean_mask is useful to mask the losses on masked tokens.
        Note: can use tf.nn.sparse_softmax_cross_entropy_with_logits and tf.reduce_mean to simplify the
                    implementation.
        Args:
