...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        Can use tf.reshape to concatenate the vectors. See following link to understand 
        ...
        """
        vecs = tf.Variable(self.pretrained_embeddings)   # need to train word vectors in the meantime
    ...
    def add_prediction_op(self):
        ...
        Note that softmax is not applied to pred here. The softmax will instead be done in
        the add_loss_op function, which improves efficiency with
        tf.nn.softmax_cross_entropy_with_logits

        Use the initializer from initialization.py to initialize W and U (can initialize b1
        and b2 with zeros)

        Dimensions of the various variables need to create:
        ...
        h_drop = tf.nn.dropout(h, self.dropout_placeholder)   # multiply constant to h_drop to be done at test time only
    ...
    def add_loss_op(self, pred):
        ...
        Can use tf.nn.softmax_cross_entropy_with_logits and tf.reduce_mean to simplify the implementation.
        Args:
...
def main(debug=False):
    ...
    with tf.Graph().as_default():
        ...
        # For an old version of TensorFlow, need to use this initializer instead.
        ...
        with tf.Session() as session:
            ...
            if not debug:
                ...
                with open('test.predicted.pkl', 'w') as f:
